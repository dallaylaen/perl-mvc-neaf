...
Start out an L<MVC::Neaf> extention by subclassing this class.
It is also used internally by Neaf itself.
Using it will make L<Carp> report the user's code as error source
instead of somewhere inside Neaf.

It also has some methods but hopefully not too many.
