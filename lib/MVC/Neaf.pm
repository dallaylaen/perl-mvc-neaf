...
that handles all the functional calls.
...
=head2 get + post '/path' => CODEREF, %options
...
=item * del for C<DELETE>

=back

These may be combined using C<+> sign, or alternatively there's
...
Dies if the same method and path combination is given again
...
=head2 $neaf->static( $req_path => $file_path, %options )
...
=head2 $neaf->set_path_defaults ( '/path' => \%values )
...
=head2 neaf "phase" => sub { ... }, path => [ ... ], exclude => [ ... ]

=head2 $neaf->add_hook ( phase => CODEREF, %options )
...
=head2 neaf alias $newpath => $oldpath
...
=head2 neaf view => 'name' => 'Driver::Class' => %options;

=head2 $neaf->load_view( $name, $object || coderef || ($module_name, %options) )
...
=head2 neaf session => $engine => %options

=head2 $neaf->set_session_handler( %options )
...
=head2 $neaf->get_routes( $callback->(\%route_def, $path, $method) )
...
=head2 $neaf->set_forced_view( $view )
...
=head2 $neaf->server_stat ( MVC::Neaf::X::ServerStat->new( ... ) )
...
=head2 $neaf->handle_request( MVC::Neaf::Request->new )
...
B<DEPRECATED>. Use C<set_error_handler> or C<neaf \d\d\d => sub { ... }>
instead.
